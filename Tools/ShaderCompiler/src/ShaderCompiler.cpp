...
bool ShaderCompiler::writeMetaFile()
{
	...
	std::string vs = "enum class VertexShaders\n{\n";
	std::string ps = "enum class PixelShaders\n{\n";
	...
	for (CompileResult& hlsl : _shaderList)
	{
		for (CompileRecord::SubShaderInfo& subShader : hlsl._record._output)
		{
			...
			str += "\t" + toUpper(subShader._name) + "," + std::string(maxLen - subShader._name.length(), ' ') + "//From: " + hlsl._fileName.getFileName() + "\n";
		}
	}
	...
	for (CompileResult& hlsl : _shaderList)
	{
		for (CompileRecord::SubShaderInfo& subShader : hlsl._record._output)
		{
			if(subShader._localParamsInfo.empty())
				continue;
